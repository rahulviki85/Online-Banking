...
public class OnlineBankingAccountTest {
   ...
   @Before
	public void initalValues() {
	   customerOne=new Customer("Rahul","123456781",new Address("Pimpri","Pune","MH"),new Account(new BigInteger("1001"),new BigDecimal(9000),new Transaction(new BigInteger("10001"),new Date(),"This is monthy payment",new BigDecimal(9000),new BigDecimal(9000))));
  ...
  public void checkBalanceAmount() {
   ...
   private boolean validateBalanceAmount(Customer customer) {
	return customer.getAccount().getBalance().compareTo(accountServives.getBalance(new BigInteger("1003")).getAccount().getBalance())==0; 
